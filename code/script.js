...
document.addEventListener('DOMContentLoaded', function() {
    ...
    profiles.forEach(profile => {
        ...
        const ageText = typeof profile.age === 'number' ? `${profile.age} anos` : profile.age;
        ...
        card.innerHTML = `
            ...
            <div class="p-4">
                ...
                <p class="text-gray-500">${ageText}</p>
        ...
        // Adiciona os dados ao próprio card para fácil acesso
        card.dataset.name = profile.name;
        card.dataset.age = ageText;
        card.dataset.image = profile.image;
        card.dataset.story = profile.story;
    ...
    function openModal(data) {
        modalTitle.textContent = `${data.name}, ${data.age}`;
    ...
    // Event listener para abrir o modal (funciona na imagem e no botão)
    ...
    // --- Lógica para copiar PIX ---
    const copyPixBtn = document.getElementById('copy-pix-btn');
    const pixKey = document.getElementById('pix-key').textContent;
    const toast = document.getElementById('toast');

    copyPixBtn.addEventListener('click', function() {
        // A API navigator.clipboard é mais moderna, mas pode não funcionar em iframes.
        // Usaremos um método de fallback.
        try {
